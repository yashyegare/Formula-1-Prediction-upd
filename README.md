...
- `flask-app/tests/` — 93 pytest tests covering auth, cookies, password reset, rate limiting, CSRF, profile endpoints (incl. display-name injection regressions), prediction scoring, and the auth contracts of both prediction endpoint families (`python -m pytest tests/ -q`). CI runs the suite against **both SQLite and PostgreSQL**, so the dual-backend query paths are exercised on every push.
- `model-notebooks/tests/` — 39 pytest tests pinning the ML training pipeline: `position_index()` bucket boundaries, the quali-vs-finish leakage fix, DNF detection, point-in-time standings ratios and recent-form windows, and an end-to-end train/predict run matching `/predictGrid`'s inference contract. CI additionally runs a **drift guard**: it rebuilds `cleaned_data.csv`/`current_roster.json` from the raw datasets and byte-compares them to the committed copies, so a stale derived artifact can't silently ship. A second guard pins the serving copy in `flask-app/` (the API's own `current_roster.json`) to the canonical build byte-for-byte, so a retrain can't leave the API's roster behind.
...
Random Forest (scikit-learn, `rffinal.pkl`) predicting a 3-class bucket (podium / points / outside points) from qualifying position and its **gap-to-pole** (best quali lap in seconds behind pole — finer resolution than the ordinal position), point-in-time driver/constructor reliability (expanding window — no future DNF data), prior-round championship standings for driver and constructor (normalized to share-of-leader so early- and late-season values are comparable), the driver's rolling last-5-race **grid→finish delta** (race-day position changes — chosen over raw average finish, which was redundant with qualifying position and carried only ~3% importance), the constructor's rolling last-5-race average points, and circuit identity. Validation is walk-forward (train on seasons ≤ Y, test on Y+1): **66.6% accuracy vs a 66.3% qualifying-position baseline, 74% podium recall, 69% points recall (baseline 63%)** — the confusion lives almost entirely at the P10/P11 boundary, so points recall is tracked explicitly alongside overall accuracy, rather than via random splits, which inflated earlier numbers to 94% through leakage. The ceiling is documented with evidence in `model-notebooks/MODEL_NOTES.md`: gap-to-pole refined the dominant feature without a step change, and LightGBM/XGBoost (including native categorical handling) scored 3–5pt *below* the Random Forest on the identical harness — the residual is race-day noise (mechanicals, incidents, safety cars) that no pre-race feature set can see. Training pipeline and experiments live in `model-notebooks/`.

**Six experiments, one ceiling.** The feature program ran to completion — each angle tested on the same walk-forward harness and recorded with its evidence in `model-notebooks/MODEL_NOTES.md`:

- **Recent-form framings** (adopted) — grid→finish delta + rolling constructor points: the only additions that moved the needle.
- **Gap-to-pole** (kept) — finer resolution of the dominant signal; no step change, but it measurably relieved load off qualifying position.
- **Model families** (rejected) — LightGBM/XGBoost scored 3–5pt *below* the Random Forest on the identical harness.
- **DNF-cause split** (null) — mechanical vs driver-error rates added nothing over the existing reliability features.
- **Circuit type** (null) — a street-circuit flag added nothing over GP identity + gap-to-pole.
- **Lap-pace + pit stops** (regressive null) — built from 201,811 real laps (2018–2026, Jolpica fetch) and lost on 4/4 seeds despite carrying 0.09 importance: recent pace was already encoded in form + quali.

The production contract is the resulting **13 features — final**. Backtested against the deployed API on real 2026 results (242 driver-race predictions, zero failures), the live service scores **71.1% vs the 68.6% qualifying baseline** — identical to the pre-experiment backtest, confirming the serving chain matches the closed contract exactly.
