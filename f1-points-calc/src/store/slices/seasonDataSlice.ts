...
export interface SeasonDataState {
  ...
  officialDriverStandings: Array<{ position: number; driverId: string; points: number }>;
  officialConstructorStandings: Array<{ position: number; teamId: string; points: number }>;
  isLoading: boolean;
  isLoaded: boolean;
...
export const fetchSeasonData = createAsyncThunk<
  { schedule: any; results: any; teams: any; drivers: any; circuitSlugs: Record<string, string>; driverStandings: any[]; constructorStandings: any[] },
  number,
...
>(
  ...
  async (year, { rejectWithValue }) => {
    ...
    const { schedule, teams, drivers, raceResults, driverStandings, constructorStandings } = await response.json();
    return { schedule, results: raceResults, teams, drivers, circuitSlugs, driverStandings: driverStandings || [], constructorStandings: constructorStandings || [] };
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSeasonData.fulfilled, (state, action) => {
        const { schedule, results, teams, drivers, circuitSlugs, driverStandings, constructorStandings } = action.payload;

        state.drivers = drivers;
        state.teams = teams;
        state.pastResults = results;
        state.officialDriverStandings = driverStandings;
        state.officialConstructorStandings = constructorStandings;

        // Transform schedule to add countryCode and completed status
        state.races = schedule.map((item: { id: string; name: string; circuitId?: string; isSprint: boolean; country: string; order: number; date: string; round: string; completed?: boolean }) => {
          // Check completed flag from API (set by Flask based on race results availability)
          const isCompleted = item.completed === true || !!results[item.id];
          return {
            id: item.id,
            name: item.name,
